...
mpc_solve::mpc_solve(ros::NodeHandle* nodehandle, int N_horz, int Ndof, int nbrCst, double dt, const std::string& panda_urdf, const KDL::JntArray& panda_q_init, const KDL::JntArray& panda_dq_init)
                     :nh_(*nodehandle),N_(N_horz), Ndof_(Ndof),nC_(nbrCst), dt_(dt),panda_urdf_(panda_urdf),panda_q_ini_(panda_q_init),panda_dq_init_(panda_dq_init)
{
    nV_ = Ndof*N_horz; // wo cao, wo ri le  !!!!!
    options_.enableFlippingBounds = qpOASES::BT_FALSE;
    options_.enableRegularisation = qpOASES::BT_FALSE; // since we specify the type of Hessian matrix, we do not need automatic regularisation
    ...
    panda_arm_ = std::make_shared<arm_kinematic>(panda_urdf, Ndof_, "panda_link0","panda_link8");
}




bool mpc_solve::initMPCData(Eigen::MatrixXd H, Eigen::VectorXd g,Eigen::VectorXd lb, Eigen::VectorXd ub)
{
    optimal_solution_.resize(nV_);
    lb_ = lb;
    ub_ = ub;
