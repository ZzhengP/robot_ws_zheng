...
//    singlePlane2 << planedata.planeLocation[1].block(0,0,5,1);
    ...
    gPlane.setZero();
    // ------------------------------------   Initialize MPC  ------------------------------------------
    Eigen::VectorXd optimalSolution,optimalSolutionPrecedent ;
    ...
    optimalSolutionPrecedent.resize(ndof*N);
    optimalSolutionPrecedent.setConstant(1);
...
//    goalStart << panda_ee_frame.p[0], panda_ee_frame.p[1], panda_ee_frame.p[2];
    ...
    int ite=0;
    std::ofstream myfile, myfile2, myfile3, myfile4, myfile5;
    ...
    myfile5.open("/home/zheng/Bureau/sqpiteration.txt");
...
//        obsAvoidanceCst.getConstraintData().print();
               ...
               std::cout <<" number of simulation iteration :\n " << ite << std::endl;
               std::cout <<" number of sqp iteration :\n " << sqpite << std::endl;
               std::cout <<" error entre deux qp iteration :\n " << (optimalSolution - optimalSolutionPrecedent).norm() <<std::endl;
               is_solved = qptest.solve();
            //      Qp for separating plane
             ...
             if (!is_solved){
                  ...
                  break;
                    ...
                    optimalSolutionPrecedent = optimalSolution;
...
//     while((q_horizon_sqp - q_horizon_sqpTest).norm()/N > 0.001);
        ...
        cubeObstacleMarkers->publishCuboid(robotVerticesPose.poses[0],0.05,obsSize[1],obsSize[2]);
        cubeObstacleMarkers->publishCuboid(robotVerticesPose.poses[2],0.05,obsSize[1],obsSize[2]);
